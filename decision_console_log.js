...
const __veri = {
    ozellik: {
        kisim:      ["yazilim"  , "donanim"     , "sistem"  , "yazilim"     , "donanim"     , "sistem"  , "yazilim"     , "donanim" , "sistem"  ],
        maas:       ["yuksek"   , "yuksek"      , "normal"  , "az"          , "normal"      , "yuksek"  , "az"          , "az"      , "az"      ],
        tecrube:    ["cok"      , "az"          , "cok"     , "cok"         , "cok"         , "cok"     , "orta"        , "az"      , "cok"     ],
    },
...
const veri = {
    ozellik: {
        ...
        para:   ["zengin"   , "zengin"  , "zengin"          , "fakir"       , "zengin"      , "fakir"       , "fakir"           , "zengin"          , "zengin"          , "zengin"  ],
    },
...
//branches = groupBy(data.ozellik["aile"]).map(item => Object.keys(item)[0])
...
while (itr) {
    ...
    if (iteration > 0) {
        ...
        if (reCalcBranches) {
            ...
            //let _calcNode = lastNode.split(":")[1] //node listesinden son düğüm noktasını hesapla
            calcNode = lastNode //node listesinden son düğüm noktasını hesapla
            let _pBranch = lastNode.split(":")[0]
            //parentBranches = _pBranch == "000" ? [] : [_pBranch]
            ...
            let __branches = groupBy(data.ozellik[calcNode.split(":")[1]]).map(item => Object.keys(item)[0])
        ...
        data = datafilter3(veri, _branches.map(item => item.split(":")[1]))
        index++
...
console.log("searchAndFixed", searchAndFixed(tree))
console.log("dede", JSON.stringify(tree, null, 2))
...
function searchAndFixed(data) {
    for (let i = 0; i < data.length; i++) {
        if (typeof data[i] == "object") {
            console.log("_root", Object.keys(data[i]))
            console.log("_data", Object.values(data[i]))
            for (let r = 0; r < Object.keys(data[i]).length; r++) {
                let _key = Object.keys(data[i])[r]
                let _value = Object.values(data[i])[r]
                delete data[i][_key]
            ...
            for (let j = 0; j < Object.values(data[i]).length; j++) {
                searchAndFixed(Object.values(data[i])[j])
            }
        }
    }
}
